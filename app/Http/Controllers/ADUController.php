...
class ADUController extends Controller
{
    public function index()
    {
        ...
        $dataOnModal = DB::select("
                    SELECT a.id, a.u_fname, a.u_mname, a.u_lname, a.email, c.astat_name, d.office, d.designation, e.c_name, f.col_name, g.ugen_name, h.utype_name
                    FROM users AS a
                    left join acceptance_log AS b
                    ON (a.id = b.u_id)
                    left join acceptance_status AS c
                    ...
                    left join user_gender AS g
                    ON (a.ugen_id = g.ugen_id)
                    left join user_type AS h
                    ON (a.utype_id = h.utype_id)
                    WHERE c.astat_name = 'Accepted'
        ");

        $temp = Auth::user()->id;

        return view('admin-disableuser', ['data' => $data, 'modal' => $dataOnModal, 'user_id' => $temp]);
    }

    public function disable(Request $request)
    {
        if($request->input('DISABLE') != null){
            $id = $request->input('DISABLE');
            ...
            foreach ($getEmail as $data) 
            {
                ...
                Mail::send('emails.disableNotif', ['gender' => $u_gender, 'lastname' => $lastname], function ($message) use ($data)
                {
                        $message->from('universitysecretary.office@gmail.com', "USeP Records Office");
                        $message->to($data->email);
                        $message->subject("Disable Account - Notification"); 
                });
            }
    ...
    {
        //diri ibutang ang function sa reenable sa admin
    }    
